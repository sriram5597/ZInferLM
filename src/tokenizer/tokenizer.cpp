...
void zinferlm::Tokenizer::build_byte_to_unicode_map_()
{
    ...
    uint32_t shifted_code_point = 256; // Start shifting unprintable bytes above U+0100
    ...
    for (int b = 0; b < 256; ++b)
    {
        if (is_printable[b])
        {
            ...
            byte_to_unicode_map_[static_cast<uint8_t>(b)] = code_point_to_utf8(b);
        }
        else
        {
            // Shift unprintable bytes into U+0100 range
            byte_to_unicode_map_[static_cast<uint8_t>(b)] = code_point_to_utf8(shifted_code_point);
            shifted_code_point++;
        }
    }
}
...
void zinferlm::Tokenizer::init_pretokenize_()
{
    ...
    pcre2_code *compiled_re = pcre2_compile(reinterpret_cast<PCRE2_SPTR>(regex.c_str()), PCRE2_ZERO_TERMINATED, PCRE2_UTF | PCRE2_UCP, &ec, &err_offset, nullptr);
    if (!compiled_re)
    {
...
std::vector<std::string> zinferlm::Tokenizer::pretokenize(std::string text)
{
    ...
    {
        int rc = pcre2_jit_match(re_.get(), text_to_split, tex_len, start_offset, 0, match_data_.get(), nullptr);
        ...
        if (rc < 0)
        {
            ...
            throw std::runtime_error("PCRE2 matching error code: " + std::to_string(rc));
        }
...
std::vector<std::string> zinferlm::Tokenizer::split_and_merge_(std::string chunk)
{
    ...
    while (true)
    {
        ...
        auto it = std::min_element(rank_pairs.begin(), rank_pairs.end(), [](const auto &a, const auto &b)
